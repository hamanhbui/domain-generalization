...
from algorithms.Metric.src.dataloaders import dataloader_factory
from algorithms.Metric.src.models import model_factory
from algorithms.Metric.src.objective_functions.hard_triplet_loss import HardTripletLoss
...
class Trainer_Metric:
    def __init__(self, args, device):
        ...
        self.triplet_criterion = HardTripletLoss(margin=0.1, hardest=False)
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            for iteration, (samples, labels, domain_labels, metric_labels) in enumerate(self.train_loader):
                samples, labels, domain_labels, metric_labels = samples.to(self.device), labels.to(self.device), domain_labels.to(self.device), metric_labels.to(self.device)

                self.optimizer.zero_grad()
                
                embedded_vectors = self.model.get_features(samples)
                
                predicted_classes = self.model(samples)
                classification_loss = self.criterion(predicted_classes, labels)
                total_classification_loss += classification_loss.item()

                triplet_loss = self.triplet_criterion(embedded_vectors, metric_labels)
                total_triplet_loss += triplet_loss.item()

                sum_loss = classification_loss + triplet_loss
